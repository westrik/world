...
pub async fn list_library_items(
    ...
    let library_items = block_in_place(move || run_get_library_items(session, &db_pool))?;
...
pub async fn get_library_item(
    ...
    let library_item = block_in_place(move || run_get_library_item(session, &db_pool, api_id))?;
    Ok(warp::reply::with_status(
...
async fn run_bulk_create_library_items(
    ...
    Ok(block_in_place(move || {
        LibraryItem::bulk_create(&get_conn(&db_pool).unwrap(), create_specs)
    })?)
}
...
pub async fn update_library_item(
    ...
    let library_item =
        block_in_place(move || run_update_library_item(session, api_id, spec, &db_pool))?;
    Ok(warp::reply::with_status(
...
pub async fn create_library_item_version(
    ...
    let library_item_version =
        block_in_place(move || run_create_library_item_version(spec, session, &db_pool))?;
    Ok(warp::reply::with_status(
