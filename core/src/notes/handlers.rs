...
pub async fn get_note(
    ...
    let note = run_api_task(move || run_get_note(session, &db_pool, api_id)).await?;
    Ok(warp::reply::with_status(
...
pub async fn create_note(
    ...
    let note = run_api_task(move || run_create_note(spec, session, &db_pool)).await?;
    Ok(warp::reply::with_status(
...
pub async fn update_note(
    ...
    let note = run_api_task(move || run_update_note(session, api_id, spec, &db_pool)).await?;
    Ok(warp::reply::with_status(
        warp::reply::json(&UpdateNoteResponse {
            error: None,
            note: Some(note),
